...
def calculate_bom():
    try:
        ...
        def process_rhombus(center_x, center_y):
            rhombus_coords = [
                (center_x, center_y - half_d), (center_x + half_d, center_y),
                (center_x, center_y + half_d), (center_x - half_d, center_y)
            ]
        ...
        if use_edge_layout:
            ...
            for i in range(-2, cols):
                for j in range(-2, rows):
                    if (i + j) % 2 != 0:
                        process_rhombus(i * half_d + total_offset_x, j * half_d + total_offset_y)
        else:
        ...
        for p in pieces:
            ...
            if abs(piece_area - full_area) < tolerance:
                ...
                gross_area_for_piece = bbox['width'] * bbox['height']
                perimeter_for_piece = 2 * (bbox['width'] + bbox['height'])

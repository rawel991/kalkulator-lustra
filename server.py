...
# Pobierz hasło ze zmiennej środowiskowej dla bezpieczeństwa
...
def calculate_bom():
    # Tutaj cała logika 'calculate_bom' z poprzedniej wersji...
    # (jest kompletna, bez zmian)
    try:
        data = request.json
        ...
        def process_rhombus(center_x, center_y):
            ...
            if not clipped_piece.is_empty and clipped_piece.area > 1e-6:
                pieces.append(clipped_piece)
        
        if use_edge_layout:
            centering_offset_x = (W % d) / 2; centering_offset_y = (H % d) / 2
            layout_shift = d / 2; total_offset_x = centering_offset_x + layout_shift
            total_offset_y = centering_offset_y + layout_shift
        ...
        for p in pieces:
            ...
            piece_area = p.area
            signature = ""; gross_area_for_piece = 0; perimeter_for_piece = 0
            if abs(piece_area - full_area) < tolerance:
                signature = f"Kwadrat {s:.1f} x {s:.1f} mm"
                gross_area_for_piece = s * s; perimeter_for_piece = 4 * s
            elif abs(piece_area - full_area / 2) < tolerance:
                signature = f"Trójkąt {s:.1f} x {s:.1f} mm"
                gross_area_for_piece = s * s; perimeter_for_piece = 4 * s
            else:
                signature = f"Docinek {bbox['width']:.1f} x {bbox['height']:.1f} mm"
                gross_area_for_piece = bbox['width'] * bbox['height']; perimeter_for_piece = 2 * (bbox['width'] + bbox['height'])
...
if __name__ == '__main__':
    # Uruchomienie serwera produkcyjnego, jeśli plik jest głównym programem
